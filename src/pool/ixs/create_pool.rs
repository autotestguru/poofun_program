use crate::{
    ...
    CreateEvent, MainState, PoolState,
};
...
pub struct ACreatePool<'info> {
    ...
    #[account(constraint = quote_mint.key().to_string() == NATIVE_MINT_STR @ PooFunError::UnknownToken)]
    ...
    #[account(
        ...
        constraint = check_balance_on_pool_creator(creator_base_ata.as_ref(), input.base_amount) @ PooFunError::InsufficientFund
    )]
    ...
    #[account(
        ...
        associated_token::authority = creator,
        constraint = check_balance_on_pool_creator(creator_quote_ata.as_ref(), input.quote_amount) @ PooFunError::InsufficientFund
    )]
