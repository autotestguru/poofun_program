use crate::{
    constants::{NATIVE_MINT_STR, ADMIN_PUBKEY}, 
    error::PooFunError, 
    MainState,
    PoolState
};
...
pub fn withdraw(ctx: Context<AWithdrawState>) -> Result<()> {
    let spec_addr = Pubkey::from_str(ADMIN_PUBKEY).unwrap();
    let admin = ctx.accounts.admin.to_account_info();
    let main_state = &ctx.accounts.main_state;
    let pool_state = &ctx.accounts.pool_state;

    require!(ctx.accounts.admin.key().eq(&spec_addr), PooFunError::Unauthorised);
    require!(main_state.initialized.eq(&true), PooFunError::Uninitialized);
    require!(pool_state.complete.eq(&true), PooFunError::BondingCurveIncomplete);
